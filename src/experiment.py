...
class ExperimentInstance:
    ...
    def calculate_metrics(self, model: Union[tf.keras.Model, BaseEstimator], history: tf.keras.callbacks.History, data_test: np.ndarray, data_valid: np.ndarray, duration: float) -> Tuple[pd.DataFrame, tf.Tensor, tf.Tensor, tf.Tensor]:
        ...
        predictions_unscaled = model.predict(data_test["data"][0])
        predictions_valid_unscaled = model.predict(data_valid["data"][0])

        if "groups" in data_test:

            true, predictions = [], []
            for group, t_unscaled, p_unscaled in zip(data_test["groups"], data_test["data"][1], predictions_unscaled):
                mean = self.scaler.scalers[group].mean_[self.label_idxs]
                std = self.scaler.scalers[group].scale_[self.label_idxs]

                true.append(t_unscaled*std + mean)
                predictions.append(p_unscaled*std + mean)
            ...
            true = data_test["data"][1]*std + mean
            predictions = predictions_unscaled*std + mean

        if "groups" in data_valid:
            true_valid, predictions_valid = [], []
            for group, t_unscaled, p_unscaled in zip(data_valid["groups"], data_valid["data"][1], predictions_valid_unscaled):
                mean = self.scaler.scalers[group].mean_[self.label_idxs]
                std = self.scaler.scalers[group].scale_[self.label_idxs]

                true_valid.append(t_unscaled*std + mean)
                predictions_valid.append(p_unscaled*std + mean)
            true_valid, predictions_valid = np.array(true_valid), np.array(predictions_valid)
        else:
            mean = self.scaler.mean_[self.label_idxs]
            std = self.scaler.scale_[self.label_idxs]

            true_valid = data_valid["data"][1]*std + mean
            predictions_valid = predictions_valid_unscaled*std + mean      
        ...
        metrics['selected_features'] = [original_features[list(self.selected_idxs)].tolist() if self.selected_idxs is not None else original_features]
        ...
        return metrics, data_test["data"][0], true, predictions
